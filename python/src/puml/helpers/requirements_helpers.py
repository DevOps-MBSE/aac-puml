...
def _get_requirements_defs(definitions: List[dict]) -> List[dict]:
    reqs = {}
    req_specs = {}
    for definition in definitions:
        if definition.get_root_key() == "req_spec":
            req_specs[definition.structure["name"]] = definition
        if definition.get_root_key() == "req":
            reqs[definition.structure["name"]] = definition
    requirements = []
    for req_key in reqs:
        req = reqs[req_key]
        id = req.structure["id"]
        attributes = req.structure["attributes"]
        type = _get_requirement_type(attributes)
        title = None
        name = req.structure["name"]
